      ...
      <main className="ml-64 p-8 relative z-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">View all your analysis reports</p>
          </div>
        </div>
        ...
        {reports.length === 0 ? (
          <div className="text-center py-32">
            <div className="w-20 h-20 bg-gradient-to-b from-amber-500/10 to-transparent border border-amber-500/20 rounded-3xl flex items-center justify-center mx-auto mb-6">
              <FileText className="w-10 h-10 text-amber-500/60" strokeWidth={1.5} />
            </div>
            <h3 className="text-2xl font-semibold text-foreground mb-2">No reports yet</h3>
            <p className="text-muted-foreground">Connect and analyze your first repository to generate reports</p>
          </div>
        ) : (
          <div className="space-y-4">
            {reports.map((report) => (
              <Link
                key={report.id}
                href={`/reports/${report.repository.id}`}
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-secondary border border-border rounded-xl flex items-center justify-center flex-shrink-0 group-hover:border-amber-500/30 transition-colors duration-200">
                    <FileText className="w-6 h-6 text-muted-foreground group-hover:text-amber-500/80 transition-colors duration-200" strokeWidth={1.5} />
                  </div>
